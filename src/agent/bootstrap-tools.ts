 ...
 * marker file to the Letta home directory so subsequent launches skip the call.
 * This backfills orgs created with an incomplete tool set (e.g., missing
...
import { getLettaHome, getLettaHomeSubdir } from "../utils/lettaHome.js";
import { addBaseToolsToServer } from "./create";

const MARKER_PATH = getLettaHomeSubdir(".bootstrapped");
...
export async function bootstrapBaseToolsIfNeeded(): Promise<void> {
  ...
  try {
    ...
    if (success) {
      mkdirSync(getLettaHome(), { recursive: true });
